<edukamu-video content="/videos/devai-7-unit3-video.yaml"/>
<br>
...
After exploring the foundational concepts of conversational language understanding (CLU), it’s time to get familiar with Retrieval Augmented Generation (RAG). While CLU equips AI systems with the ability to understand and respond to user queries, RAG takes this a step further by enhancing the generation of responses.

Although retrieval augmented generation might sound difficult or even confusing at first, you’ll certainly learn to understand its basic principles as we connect it with topics we’ve already covered before.

The connection between CLU and RAG, for example, lies in their synergy: CLU provides the understanding, and RAG enriches the generation, resulting in more contextually relevant and informative responses.
...
Retrieval Augmented Generation, or RAG, represents a paradigm shift in natural language processing, bringing about exciting advancements in the generation of human-like responses. At its core, RAG combines elements of information retrieval and text generation to produce highly context-aware and informative responses.

### Key Benefits of RAG

1. **Enhanced Responses**

RAG focuses on improving the quality of responses generated by AI systems. It achieves this by retrieving relevant information from large corpora of text or knowledge bases and using that information to enrich the generated content.
...
<span style="font-weight: bold">Many Use Cases</span>
</li>
</ol>
</edukamu-section>

RAG finds applications across various domains. It's particularly valuable in question-answering systems, content generation, virtual assistants, and chatbots. For instance, it can be used to retrieve up-to-date information for news articles, provide detailed explanations for user queries, or even create personalized responses based on historical interactions.

<edukamu-section class="styled-list-not-in-order">
<ol> 
<li style="counter-increment: any-name 3;"> <!--Merkataan any-namen perään numero mikä halutaan non-ordered listaan tähän kohtaan-->
<span style="font-weight: bold">Knowledge Integration</span>
</li>
</ol>
</edukamu-section>

RAG seamlessly integrates external knowledge into the generation process. This external knowledge can be drawn from diverse sources like the web, encyclopedias, or domain-specific databases, ensuring that the responses are not only contextually relevant but also factually accurate.

<edukamu-section class="styled-list-not-in-order">
<ol> 
<li style="counter-increment: any-name 4;"> <!--Merkataan any-namen perään numero mikä halutaan non-ordered listaan tähän kohtaan-->
<span style="font-weight: bold">Complements CLU</span>
</li>
</ol>
</edukamu-section>

RAG complements traditional CLU capabilities by addressing one of its limitations: the generation of informative responses. While CLU enables systems to understand user queries, RAG steps in to enhance the content generation aspect, ensuring that the responses are not just contextually accurate but also rich in content.

In essence, Retrieval Augmented Generation represents a powerful evolution in the field of natural language processing. It empowers AI systems to not only understand user queries but also provide detailed, contextually relevant, and informative responses by tapping into vast repositories of external knowledge.

Retrieval augmented generation has the potential to revolutionize various applications, from customer support chatbots to content generation platforms, delivering a more engaging and informative experience for users.

</edukamu-collapse>
<br>

In Microsoft Azure, retrieval augmented generation is included in a service called Azure AI Search. Let’s find out more.
...
<edukamu-image url="/graphics/module3/travel-agent.png" alt="A diagram showing how a travel agent searches for information to help customer plan a trip.">
<br>
...
When using Azure AI Search, there are a few aspects that need to be carefully taken into account at every stage of development. Namely, these include 1) managing capacity, 2) understanding search components, 3) indexing, 4) filtering and sorting, and 5) enhancing the indexes.

Next, we’ll review each of these aspects.
</edukamu-section>
<br>
...
That, in a nutshell, is how Azure AI Search can help you unlock the hidden insights within your data. Keeping in mind the aspects covered before, let’s delve deeper into retrieval augmented generation. This time, there won’t be any exercises to complete, so feel free to move right ahead!
